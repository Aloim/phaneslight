...
---
<!-- PhanesLightUpgrade v3.7.0, 2026-09-03, production upgrade prompt for existing PhanesLight installations.
...
<!-- LEGACY-NAME-BLOCK BEGIN: the identifiers below are pre-v3.6.0 names, matched literally against what is on disk. Do NOT rename them. -->
   ...
   * **Reinstall the script library** from the v3.7.0 manifest into `.phaneslight/scripts/`, which is what brings in the renamed dispatcher and the new `phaneslight-template` stamps, then smoke-run it: `node .phaneslight/scripts/cli.js module-list`. Do **NOT** delete `.phanes/scripts/` yet.
