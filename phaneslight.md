<!-- PhanesLight v3.8.1 (2026-09-05). Single-file bootstrap prompt.
     Installed copies check this stamp against upstream on every run (Phase 0 Step 0) and offer /phaneslightupgrade when a newer version has shipped.
...
**(v3.8.0) Before the reviewer is dispatched, the orchestrator runs `phaneslight anchor-check --plan <plan>` over the plan and over the handover it was launched from.** The split of who owns which verdict is not arbitrary. `MOVED`, `MISSING`, `MISMATCH` and `WILL-MOVE` are handed to the reviewer as findings, because they are claims about the repository that have stopped being true and grading them is review work. `UNVERIFIABLE` and `AMBIGUOUS` are the **orchestrator's own to resolve first**, because they are defects in its own writing: an anchor with no expectation, or a path token that names several files, is a sentence the orchestrator wrote badly, and sending it to the most expensive tier to be told so is waste. A plan whose anchors have gone stale will generate defects from correct instructions, which is the failure this command exists to catch: a brief that says "do not move it from 53" against a constant the file reads as 52 turns an obedient worker into a defect generator, and the control then asserts by count and goes red on the obedience.
...
1. Run `phaneslight regen-registry`, then `phaneslight api-diff <last-phase-ref>`. Cross-check the result against the active plan's API-changes section and report **planned-and-found**, **planned-and-missing**, and **unplanned additions**. An unplanned change is drift even when it compiles. **(v3.8.1)** `api-diff` fails closed: a non-zero exit is a failed measurement and not a clean surface, so name the cause and stop rather than reading an empty diff as no drift.
...
**(v3.8.0) Every `file:line` in a handover carries the expectation that re-finds it, and a PREDICTED line number is never written down.** Write `path:N "<substring expected on that line>"`, not a bare `path:N`, and never a line number computed from an edit that has not been made yet. The successor measures; the predecessor states what to measure for. This is the rule that would have prevented the largest single observed class of stale anchor, and the reason it belongs HERE is that the ceiling is exactly where it gets broken: an agent writing a handover is summarising a tree it is about to stop being able to see, under time pressure, and a line number recalled rather than measured looks identical on the page to one that is correct. Run `phaneslight anchor-check` over the handover before closing.
...
**IMPERATIVE:** Before any other action, before the run-state marker, before any pre-flight, **YOU MUST** verify this prompt is the newest published PhanesLight. A stale spec must never bootstrap or update a project.

1. **Read your own version** from the stamp on line 1 of this file (`<!-- PhanesLight vX.Y.Z, ... -->`).
2. **Fetch the upstream stamp.** Download the published file to a temporary location and read its line-1 stamp. **The distribution repository is `Aloim/phaneslight`.** PhanesLight moved there from `Aloim/phanes`, which is being handed to a separate and more sophisticated Phanes project that inherits the name. Detect the platform **FIRST** and run only the matching variant:

   **POSIX (bash/zsh):**

   ```
   curl -fsSL https://raw.githubusercontent.com/Aloim/phaneslight/main/phaneslight.md -o /tmp/phaneslight-upstream.md && head -1 /tmp/phaneslight-upstream.md
   ```

   **Windows (PowerShell 5.1+):**

   ```powershell
   try { Invoke-WebRequest -Uri https://raw.githubusercontent.com/Aloim/phaneslight/main/phaneslight.md -OutFile "$env:TEMP\phaneslight-upstream.md" -ErrorAction Stop; Get-Content "$env:TEMP\phaneslight-upstream.md" -TotalCount 1 } catch { Write-Output "FETCH-FAILED" }
   ```
...
3. **Compare numerically.** Parse `v<major>.<minor>[.<patch>]` from both stamps (a missing patch component counts as 0) and compare component-wise. **NEVER** compare version strings lexically.
   ...
   * **Upstream newer →** sanity-check the download first: the file **MUST** begin with `<!-- PhanesLight v`. A 404 body, an HTML error page, or a truncated fetch must never be treated as a release, if the check fails, treat it as a fetch failure below. Then **ASK the user** (verbatim, substituting real versions):

     > "A newer PhanesLight is published: vX.Y.Z (installed) → vA.B.C (upstream). Upgrade now? The upgrade runs as its own command, touches only PhanesLight-owned artifacts, and preserves all project knowledge. (yes / no)"

     * **User answers yes →** **STOP** immediately and tell them (verbatim): "Run `/phaneslightupgrade` to bring this installation to vA.B.C. This run has made no changes and the run-state marker was not touched." **DO NOT** overwrite any installed command file yourself, the upgrade command owns every file replacement. **(v3.8.1) An installed but stale `/phaneslightupgrade` is not a hazard here, because the upgrade command refreshes ITSELF and stops**: its Step 2 compares the installed `phaneslightupgrade.md` line-1 stamp against the upstream version and, where upstream is newer, archives the installed copy, replaces it, and stops, so the newest upgrade logic is the logic that performs the upgrade. From v3.8.1 that self-check runs before the command replaces anything else; a v3.8.0 or older installed copy replaces `phaneslight.md` from upstream first and then refreshes itself, which lands the same target either way. Do not add a second version check here. What this branch still owes is the ABSENT case: if `/phaneslightupgrade` is not installed, give the fetch command for the platform first: `curl -fsSL https://raw.githubusercontent.com/Aloim/phaneslight/main/PhanesLightUpgrade.md -o ~/.claude/commands/phaneslightupgrade.md` (Windows: `Invoke-WebRequest -Uri https://raw.githubusercontent.com/Aloim/phaneslight/main/PhanesLightUpgrade.md -OutFile "$env:USERPROFILE\.claude\commands\phaneslightupgrade.md"`).
...
4. **(v3.8.0) Reconcile against the project's own recorded version, separately from the upstream check above.** The upstream check above asks whether a newer PhanesLight has shipped; this step asks whether the version you are running is even compatible with the state this project already carries. Read `phanesLightVersion` from `.phaneslight/config.json` (absent on a fresh bootstrap, nothing to reconcile, proceed) and the declared `migrationBoundaries` array from `templates/MANIFEST.json` (fetched below, in Acquire, do not author; where the template library has never been fetched, this reconciliation has nothing to compare against and is skipped). A **migration boundary** is a version at or above which the project structure changed enough that an in-place update is not sufficient; a boundary is *crossed* when the project's recorded version is below a boundary and yours is at or above it. Parse `v<major>.<minor>[.<patch>]` from both (a missing patch counts as 0) and compare numerically, never as strings, then route on the table:

   | Condition | Action |
   |---|---|
   | No `.phaneslight/config.json` | Fresh bootstrap. Proceed. |
   | Equal versions | Normal update run. |
   | Project older, no boundary crossed | Normal update run. |
   | Project older, a boundary crossed | **STOP.** Direct the user to `/phaneslightupgrade`. |
   | **Project recorded newer than the version you are running** | **STOP and ASK.** Never regenerate. |
   | Version present but unparseable | **STOP and ASK.** Never guess. |

   **On a crossed boundary, say this and stop:**

   > "This project was generated by PhanesLight vX.Y.Z. You are running vA.B.C, and the structure changed between them. Run `/phaneslightupgrade` first; it migrates the project behind a generated, evidence-verified checklist and then hands back here. This run has made no changes and the run-state marker was not touched."

   **On a project recorded newer than you, say this and stop.** This branch is new at v3.8.0 and a future run **MUST NOT** read it as excessive caution:

   > "This project records PhanesLight vX.Y.Z, which is newer than the vA.B.C I am running. I will not regenerate it."

   The reasoning: a project recorded above the version you are running means this installed copy moved backwards, not the project forwards, a stale local cache, a pinned older fetch, or a project last touched from a machine that had already fetched a newer release all produce it. Regenerating that project's state with an older generator destroys structure the older generator cannot even identify. This is the same failure mode the legacy-name gate below refuses, and it gets the same answer: stop, and ask. **Refusing a legitimate project costs one clarifying answer. Adopting one you cannot describe starts a migration that cannot be finished or undone.**

   **On an unparseable recorded version, say this and stop:**

   > "This project's recorded PhanesLight version, '<value>', does not parse as vX.Y.Z. I will not guess at what it means and will not regenerate until this is resolved."

   This step never overwrites any installed command file itself, exactly as the upstream check above does not; the upgrade command owns every file replacement.

**The repository migration, and the one rule that keeps it safe (v3.6.1, restated at v3.7.1, changed at v3.8.0).** v3.6.1 was published to **both** `Aloim/phaneslight` and the legacy `Aloim/phanes`, so that installations still checking the legacy URL could see the release and migrate themselves onto the new repository. v3.7.1 was published to both for the same reason. **That is no longer the policy: from v3.8.0 the manual line publishes to `Aloim/phaneslight` alone.** The legacy `Aloim/phanes` is frozen at v3.7.1 and will not change again, full stop, a separate and more sophisticated Phanes project inherits that repository and its name. An installation whose configured upstream is still the legacy URL sees v3.7.1 there, is redirected onward by that file, and upgrades from there onto `Aloim/phaneslight`; from then on it checks the correct repository.

**The plugin and the manual prompt are now separate products at separate repositories, and this file is the manual one.** If you installed PhanesLight through `/plugin marketplace add`, the plugin's marketplace moved to `Aloim/phaneslightplugin` and you re-add it there; nothing in this file applies to that installation. If you installed by fetching `phaneslight.md`, this file is your upstream and `Aloim/phaneslight` is where it lives.

**The stamp check is what makes this safe, and it is not optional.** The legacy repository is now frozen and will one day hold a *different product* entirely, so a fetch from it may one day return something that is not PhanesLight at all: a frozen repository that will one day hold something else is exactly the case this check exists to guard against. The line-1 requirement, the file **MUST** begin with `<!-- PhanesLight v`, already covers exactly this: a file failing it is treated as a fetch failure, never as a release, whatever version number appears inside it. **Never relax that check to accommodate a rename, and never infer a version from a filename or a URL.**
...
2. **Update Run (marker present):**
   ...
   * **Sense, then regenerate (v3.4).** An update run no longer starts by regenerating everything. It starts by **measuring what moved**. Run `node .phaneslight/scripts/cli.js update-preflight --spec-version <your own stamp version from Step 0>` and read its JSON verdict; the script is offline, advisory, and always exits 0, and it reports only what it can observe. **The flag is mandatory:** without it the spec sensor reports no delta at all and can never gate, so a consumer whose other sensors are quiet takes the verify-only branch below and never regenerates its agents after a version bump. Take exactly the branch its verdict names:
...
```
**Installed Capability Register (GENERATED, regenerated by every /phaneslight run; hand-editing FORBIDDEN):**
...
**Archive digest template** (`<projectSlug>-closure`'s fixed output shape, ≤15 lines, no narrative prose; plan paths and SS references are copied **verbatim** from the original entry, and **(v3.8.0) only lines that pass `anchor-check` are copied**: a pointer that has gone stale is re-anchored before it is carried into the digest, never copied faithfully into a permanent record where nothing will ever check it again: they are the recovery paths back to full detail, and `CLAUDE.local.md` is conventionally gitignored, so git history cannot be assumed to preserve the original):
...
* **`phaneslight anchor-check [--plan <file>] [--strict] [file ...]`** (v3.8.0; one Node program with a launcher on each platform, like `cli.js`; advisory, always exits 0 unless `--strict`), resolves the precise pointers a plan, brief or handover uses against the **working tree, never HEAD**, and names the ones that have gone stale. An **anchor** is a `file:line`, a block range, a measured line count or a numeric constant. It goes stale when it was accurate when written and the tree then moved underneath it: the pointer still resolves, still looks authoritative, and names something else, so there is no compiler to reject it and the failure is silent. Verdicts: `OK`, `MOVED (found at :K)` with K the re-anchoring, `MISMATCH (<actual>)`, `MISSING`, `UNVERIFIABLE`, `AMBIGUOUS`, and `WILL-MOVE` in plan mode. **A bare `file:line` carrying no expectation is `UNVERIFIABLE`, not `OK`**, which is the point: it names the anchors that cannot be checked so they get rewritten rather than passing silently. With no positional file it reads `CLAUDE.local.md` and the highest-numbered `SS*` file under `<docRoot>/session-summaries/`. `--strict` exits 1 on anything worse than `OK`. It reports what moved; it **cannot** tell you whether the instruction built on the anchor is still right, and that judgment stays with the reviewer tier.
...
* **`phaneslight api-diff <since-ref>`**, diffs the current API surface against a git ref or a saved baseline. For a git ref, extract the old surface from that ref's *source*, never depend on historical baseline files existing in git (`.phaneslight/` may be untracked). Outputs structured report: added, removed, changed signatures, with file references. **(v3.8.1) `api-diff` MUST fail closed.** Any extraction failure, missing baseline slice, or unresolvable ref is a non-zero exit with the cause named on stderr, never an empty or partial diff. An empty diff from a failed extraction is the drift instrument reporting clean without having measured, which is the one failure mode a drift instrument must not have. A caller reading a non-zero exit treats the surface as **UNMEASURED**, never as unchanged. `api-diff` is `generatedNotFetched`, so this paragraph is the contract the project's own generated script is written against, and the failure rule is as binding as the output shape.
...
* **`phaneslight preflight`** (advisory, always exits 0), the Phase 0 mechanical pre-flight in one call. Reads the `.claude/.phaneslight` marker and `phanesLightVersion` from `.phaneslight/config.json`, fetches the upstream stamp (network-tolerant: on any failure `upstream` is `null`, never a guess), runs `claude mcp list` and reports the four standard servers, detects the platform, and enumerates the disk-visible capability surfaces. Emits one digest JSON: `{runType, marker, markerReadable, configReadable, rootSource, installedVersion, upstream, mcp: {context7, deepwiki, serena, semble}, platform, censusCounts, censusUnreadable, legacyMarkers}`. `runType` is `setup`, `update`, `anomaly` or **`unknown`**, the last meaning the marker exists and could not be read, which is a question for the user and never a licence to run setup. A surface it could not enumerate reports `null` and names itself in `censusUnreadable`, never `0`: **an unreadable surface is not an empty one.** When `.claude/agents` itself is one of the surfaces named there, the two derived counts inside `legacyMarkers`, `unprefixedTemplateAgentsCount` and `sequentialThinkingAgentsCount`, are `null` rather than `0`, and `listTruncated` reports `false`, since a list derived from a directory that was never read carries no truncation to report. `legacyMarkers` carries the legacy-migration signals so the **session**, not the script, executes the STOP-and-route-to-`/phaneslightupgrade` judgment. Consent, `AskUserQuestion`, MCP installation and every verdict stay in the session; the script probes nothing requiring auth interaction and **mutates nothing**. Consumed by Phase 0.
...
* **The hook sensor inside `update-preflight --hooks-only`** (v3.8.0 correction). There is no `phaneslight hook-verify` command and there never was a manifest row for one: the name always described a SENSOR that `update-preflight` runs, and `update-preflight`'s own header says so. It was documented here as a command bullet for several releases and nothing noticed, which is exactly the defect v3.8.0's guard 10 exists to refuse. It reports the hook table's state (present, missing, mis-pathed) by reading the entries in the project's own `.claude/settings.json` against the scripts on disk, and it repairs nothing. Repair is Step 4b's duty, and it stays there because adding a hook is a write to the user's own `.claude/settings.json`.

Write `.phaneslight/config.json` with the confirmed module list, primary language, build system, hook preferences, language-specific extractor configuration, the runtime fields the fetched scripts read (`commentSyntax`, `docRoot`, `stampedTrees`), the optional `doc_discipline` block, the `templates` provenance block, the `lastRun` block written at each Phase 5 close-out (**v3.4**, the substrate `update-preflight`'s `gitDelta` sensor measures against; absent on a project that has never closed a run, which simply costs the next run its fast path), and the `capabilities` block, the durable memory of the Capability Census & Consent Gate (Phase 0): the per-item `selection[]` (what the user consented to build policy around, with reachability), the `granted[]` matches, and the `failures[]`. Set `commentSyntax` to the detected language's line comment marker (for example `//`, `#`, or `--`), since `new-file` stamps source files with it; set `stampedTrees` to the trees the stamp guard protects (the source roots plus `tests` and the documentation root). **(v3.8.1) The guard reads `stampedTrees` as authoritative, on both platforms: when it is present and non-empty, the guarded set is exactly the listed trees plus `docRoot`, and `modules` never extends it; when it is absent or empty, the guarded set is `src`, `tests` and `documentation`, plus `docRoot`, plus every `modules` entry. `docRoot` is the one floor an explicit list does not remove: the DOC header it guards is what `doc-index` and `doc-check` read, so listing the documentation root is a restatement of the rule, never a choice.** Through v3.8.0 `modules` was appended unconditionally on both ports, so a project that deliberately excluded a tree had no knob that made the exclusion hold, and the two ports disagreed on the base set besides: Windows replaced the defaults with `stampedTrees` while POSIX only ever added to them. Set `templates.source` to `"fetched"` when the templates were acquired from the distribution repository, `"generated"` when the fallback produced them, or `"preserved"` when an update run kept an existing project shaped library instead of fetching over it (the Reconcile clause above):
...
**(v3.8.0)** `anchorCheck` is optional and follows `doc_discipline`'s precedent exactly: the block is optional, every key inside it is independently optional, and it is read by scripts rather than by an agent. `controlRoots` is a list of directories to scan for line-pinned control keys, and when declared it **REPLACES** the defaults (`tests`, `test`, `__tests__`, `spec`) rather than adding to them. `obligations` is a list of `{ glob, mustAppearIn[], mustContain }` records applied to files a plan names as new or moved. Both default to empty, and `anchor-check` **states the default it used in its own summary line** rather than assuming you know it: a default nobody can see is a default nobody can correct. With nothing declared it prints `pins: 0 under tests, test, __tests__, spec` and `obligations: none declared`, and a sensor with nothing to say printing its silence explicitly is the point: silence and "I did not look" must never render identically. There is deliberately no `pinPatterns` key, because a list of extra regexes would be an anchor table of its own and would go stale the same way. **This block must stay listed in this schema:** `.phaneslight/config.json` is MIGRATE class, and MIGRATE carries values into the TARGET SCHEMA, so a key the schema does not list has no guaranteed survival across an upgrade.

**(v3.4)** `doc_discipline` is optional, and both of its keys, `index_exclusions` and `frozen_classes`, are independently optional, each a list of folder entries. `index_exclusions` is read by both `doc-index` and `doc-check` (`Get-DisciplineList`/`Test-Excluded` on Windows, `norm_entries`/`is_listed` on POSIX); `frozen_classes` is read by `doc-check` only, `doc-index` never references it. Every clause of the normalization rule is identical across platforms **except case matching**: entries are docRoot-relative; a leading `docRoot/` segment is stripped if the author wrote one; `\` separators normalize to `/`; entries that end up empty, whitespace-only, or slash-only after trimming are dropped. An entry **containing** a slash matches as a docRoot-relative path prefix, that folder and everything beneath it; an entry with **no** slash matches any folder of that name at **any** depth under `docRoot`. Case matching diverges by platform, deliberately, each follows its own filesystem's convention: Windows matches case-insensitively (`Get-DisciplineList`'s prefix strip uses `OrdinalIgnoreCase`; `Test-Excluded` and `Is-Frozen` use PowerShell's default case-insensitive `-eq`), POSIX matches case-sensitively (`norm_entries`'s prefix strip and `is_listed`'s comparisons are shell `case` patterns). An entry `"Reports"` matches a folder named `reports` on Windows but not on POSIX; write entries in the same case as the folder on disk and the rule holds on both. The built-in exemptions are scoped by check, not blanket. `archive` is hardcoded out of index generation itself (`doc-index.ps1`'s `if ($name -eq 'archive') { return }`; `doc-index.sh`'s `find ... ! -path '*/archive'`) and is also exempt from `doc-check`'s content checks. `session-summaries` and dated folders (`YYYY-MM-DD...`) are hardcoded only into `doc-check`'s content-check exemption (`Is-Frozen`/`is_frozen`); they are otherwise indexed normally, a `session-summaries/` folder with no `_index.md` still gets flagged `NO-INDEX` unless it is separately listed in `index_exclusions`. That is intended: content-frozen and index-exempt are different guarantees. `doc_discipline` only ever adds entries to these built-ins, never replaces them. **(v3.8.1) Every one of these tests applies to DIRECTORY segments only, on both platforms**: the date test, the `archive` and `session-summaries` tests, a slashless `frozen_classes` entry, and a slashed one, which is a folder prefix matched against the directory path. A frozen class is a folder, and a file's own name never freezes it. Through v3.8.0 both ports matched the date pattern against the whole relative path, the file name included, so a living plan under the conventional `<YYYY-MM-DD>_<topic>.md` name was silently exempt from both the ceiling and the DOC-header check, and `doc-check: OK` was vacuous for that entire class; Windows additionally tested `frozen_classes` against the file name where POSIX tested the directory alone. **The two keys are deliberately asymmetric.** `index_exclusions` suppresses `doc-index` generation for the listed trees and, in `doc-check`, suppresses the `NO-INDEX` and `STALE-INDEX` complaints for them, since no index is expected there, but it does **not** suppress `OVER-CEILING` or `NO-DOC-HEADER`: exclusion means "do not index this tree," not "do not audit its content." Suppressing content checks is what `frozen_classes` is for. A project wanting both for the same tree lists it in both keys.
...
"anchorCheck": {
  "controlRoots": ["tests"],
  "obligations": [
    { "glob": "src/commands/**", "mustAppearIn": ["README.md"], "mustContain": "# Usage:" }
  ]
},
...
Generate the hook scripts in `.phaneslight/scripts/` (platform-appropriate, shell on POSIX, PowerShell or a cross-platform runner on Windows), then **MERGE** the following into the project's `.claude/settings.json`, never overwrite existing settings; preserve any hooks already present:
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Write",
        "hooks": [{ "type": "command", "command": ".phaneslight/scripts/hook-stamp-guard.sh" }]
      }
    ],
    "PostToolUse": [
      {
        "matcher": "Write|Edit",
        "hooks": [{ "type": "command", "command": ".phaneslight/scripts/hook-size-check.sh" }]
      }
    ]
  }
}
...
**(v3.4) A third hook, `SessionStart`, ships in the fetched settings fragment on both platforms (v3.7.2 gave `hook-ledger-status` its POSIX sibling, closing the gap this line described before that release).** Each platform's settings fragment carries its own form of the entry, invoking that platform's script through its own interpreter. The entry has no `matcher` (SessionStart takes none):
...
"SessionStart": [
  {
    "hooks": [{ "type": "command", "command": ".phaneslight/scripts/hook-ledger-status.sh" }]
  }
]
```
...
Report the installed hooks in the bootstrap session summary. On update runs, verify the hook entries still exist and still point at existing scripts via project relative paths; repair silently deleted entries, rewrite any command that has been anchored at an absolute or out of project path back to its relative form (the Path discipline check above), and report every repair. **(v3.4)** This repair duty explicitly **ADDS** a missing `SessionStart` entry on a project installed before its platform's `hook-ledger-status` shipped (v3.4 on Windows, v3.7.2 on POSIX): a hook class that did not exist when the project was installed is a repairable **absence**, not a user deletion, and the two are distinguishable precisely because the project's `phanesLightVersion` predates the class. The mechanical trigger is `update-preflight`'s hook sensor, which reports the table state; the repair itself follows the same merge-never-overwrite rule as the initial install.
...
2. **Apply the Ladder Rules**
   ...
   * The orchestrator's definition states that it is the main executor, not only a dispatcher. **(v3.8.0)** It also states that on a planned launch its first act is `phaneslight anchor-check --plan <plan>` over the plan and the handover, BEFORE the reviewer is dispatched, with `MOVED`, `MISSING`, `MISMATCH` and `WILL-MOVE` handed on as reviewer findings and `UNVERIFIABLE` and `AMBIGUOUS` resolved by the orchestrator itself as defects in its own writing (§IV, Plan review at launch). The generated file must carry this, not merely the prose the primary reads: the agents are REGENERATE class, so regeneration is what proves the text travelled.
