...
**The repository migration, and the one rule that keeps it safe (v3.6.1, restated at v3.7.1).** v3.6.1 was published to **both** `Aloim/phaneslight` and the legacy `Aloim/phanes`, so that installations still checking the legacy URL could see the release and migrate themselves onto the new repository. **v3.7.1 is published to both for the same reason**, because the manual line's home has changed again: `Aloim/phaneslight` is now the manual distribution repository, and the Claude Code plugin has moved to `Aloim/phanesplugin`. An installation whose configured upstream is still the legacy URL therefore sees v3.7.1 there once, upgrades, and is repointed by `/phaneslightupgrade` as part of that upgrade; from then on it checks `Aloim/phaneslight`.

**The plugin and the manual prompt are now separate products at separate repositories, and this file is the manual one.** If you installed PhanesLight through `/plugin marketplace add`, the plugin's marketplace moved to `Aloim/phanesplugin` and you re-add it there; nothing in this file applies to that installation. If you installed by fetching `phaneslight.md`, this file is your upstream and `Aloim/phaneslight` is where it lives.
