<!-- PhanesLightUpgrade v3.8.1, 2026-09-05, production upgrade prompt for existing PhanesLight installations.
     Upgrades: ANY installed PhanesLight version → the latest phaneslight.md published at github.com/Aloim/phaneslight.
     Companion to phaneslight.md. Version jumps run here; same-version refreshes are /phaneslight update runs.
...
## 0. Read this first if you are a human, not the run

**This file is the manual upgrade prompt.** It belongs at `~/.claude/commands/phaneslightupgrade.md` (or `.claude/commands/` inside one project) and is invoked as `/phaneslightupgrade`. Everything from section I onward is addressed to the agent executing the upgrade, not to you. This section is the part addressed to you, and it is the only part you need.

### Getting the command files in the first place
...
Fetching the upgrader alone is enough: Phase U0 Step 2 below refreshes `/phaneslight` for you.

### Upgrading a manual install: v3.8.0 to v3.8.1, v3.7.1 to v3.8.1, or v3.6.1 to v3.8.1

v3.8.0 is the last manual release before this one, so upgrading from it is the common case, and it is the smallest jump this prompt handles: v3.8.1 is a patch release that replaces two scripts and the spec paragraphs describing them. v3.7.1 to v3.8.1 is the same shape one release larger. v3.6.1 to v3.8.1 is also a fully supported path, and not a smaller variant of the common case: as the next section explains, v3.6.1 sits below a boundary this release still declares, so it is routed through the same run rather than through an ordinary update.

**There is no manual v3.7.0.** v3.7.0 was the plugin release, published to `Aloim/phaneslight` by mistake; it was built for the Claude community marketplace and has since moved to `Aloim/phaneslightplugin`. Its retirement notice, which told you this path was closing, is withdrawn. A v3.6.1 install goes straight to v3.8.1 and skips nothing that applies to it.

**There is no manual v3.7.2 either.** v3.7.2 was a plugin-only release (POSIX parity for the ten bootstrap commands); the manual line never published it, and its content is folded into this file's `Changelog.md` v3.8.0 entry rather than given a manual number of its own. A v3.7.1 install goes straight to v3.8.1 the same way.
...
3. **Run it on Opus 5 at `high` effort** (`claude --model opus --effort high`). See the note in section I: this is the worst run in the library to economize on.
4. **Restart your Claude Code session**, then run `/phaneslight` once.

**From v3.8.0**, the jump is a patch: `migrationBoundaries` is unchanged and no boundary is crossed, so this is an in-place update. The work is confined to reinstalling the template library at its `v3.8.1` stamps, which replaces `hook-stamp-guard` and `doc-check` on both platforms, and to the two behaviour changes the `Changelog.md` v3.8.1 entry states under Installed project impact. Expect `doc-check` to report living documents whose names begin with a date: those findings were always true, the checker was not looking.

**From v3.7.1**, the jump is small and additive: `migrationBoundaries` in the v3.8.1 manifest reads `["3.5.0", "3.7.0"]`, and v3.7.1 sits above the highest entry already, so nothing in your project structure moves on this path. The work is confined to regenerating `<projectSlug>-mechanic` and `<projectSlug>-reviewer` for the two v3.7.1 rules, adding the anchor convention sweep, restating them in the root `CLAUDE.md` Pinned Directives block, and reinstalling the template library at its `v3.8.1` stamps, which also carries the v3.8.1 script fixes. Accumulated knowledge is untouched.

**From v3.6.1**, the picture is different: v3.6.1 sits below the manifest's `3.7.0` boundary entry, so this jump crosses it. A project at v3.6.1 or below is not upgraded in place by an ordinary `/phaneslight` update run; it is routed through this same `/phaneslightupgrade` procedure instead, and the run performs a full regeneration rather than the narrow v3.7.1-only work described above. The steps you run are identical either way, the run detects which case it is looking at and plans accordingly.

**Upgrading from v3.4.1 or earlier** is a larger jump still, one that also carries the rename (`.phanes/` to `.phaneslight/`) and the repository migration. Same three steps; the run detects the version itself and plans accordingly.

### Reversing a plugin install, back to the manual one

Do this if you installed the plugin and would rather have the single-file prompt, or want your hooks recorded in your own `.claude/settings.json` instead of registered by a plugin. **Do not run this prompt to do it**; this prompt upgrades a manual install, it does not uninstall a plugin. Do the five steps below by hand first, and only then run `/phaneslight`.

1. **Uninstall the plugin and drop its marketplace.** The plugin and the marketplace are both named `phaneslight`, whichever repository you added it from:
   ...
   Non-interactively: `claude plugin uninstall phaneslight@phaneslight -y` and `claude plugin marketplace remove phaneslight`. Confirm with `/plugin list` and `/plugin marketplace list`.

2. **Install the manual command files** with the fetch commands at the top of this section. You want both `phaneslight.md` and `PhanesLightUpgrade.md`.

3. **Restart Claude Code.** Hook configuration is read at session start, so do not judge the state of anything until you have restarted.

4. **Run `/phaneslight`.** This step is not optional and it is the one people skip. When the plugin took over hook registration it **removed PhanesLight's hook entries from your project's `.claude/settings.json`**, so between uninstalling the plugin and completing a manual run, *nothing is enforcing stamp and size discipline*. The manual run re-merges the settings fragment, reinstalls the script library into `.phaneslight/scripts/`, and regenerates the roster.

5. **Restart once more.** The hooks you just registered are themselves snapshotted at session start.

**Your project state carries over untouched, in both directions.** Manual and plugin are numbered independently from v3.7.0 on: the manual skipped v3.7.0 and v3.7.2 outright and lands on v3.8.0 directly from v3.7.1, while the plugin passed through both. The two lines happen to share the `v3.8.0` number again after this release, but that is coincidence, not a promise: they are separate products published from separate repositories, and a later release can move one without the other. What holds regardless of whether the numbers match: both use `.phaneslight/`, the same `phanesLightVersion` key, the same manifest shape and the same five agents at this snapshot. Neither direction is a migration and neither trips the version gate.

**Do not run both at once.** A plugin install plus manual command files leaves two live entry points at the same version, and Claude Code does not document which wins on a name collision. Finish whichever route you chose.

### Going the other way, manual to plugin
...
Then restart and run `/phaneslight:upgrade`.

---

## I. Identity and Objective

**The run starts here. Section 0 above is documentation for the human and is NOT part of this procedure**: never execute its shell commands, never uninstall or install a plugin, and never fetch a command file except where Phase U0 Step 2 tells you to. If what the user needs is the plugin reversal in section 0, your job is to say so and stop, per Phase U0 Step 1.
...
Fetch upstream `phaneslight.md` **AND** `Changelog.md` to temporary locations. **The distribution repository is `Aloim/phaneslight`**, moved from the legacy `Aloim/phanes`, which is being handed to a separate and more sophisticated Phanes project. Detect the platform **FIRST** and run only the matching variant:
...
curl -fsSL https://raw.githubusercontent.com/Aloim/phaneslight/main/Changelog.md -o /tmp/phaneslight-changelog.md && head -1 /tmp/phaneslight-changelog.md
```
...
try { Invoke-WebRequest -Uri https://raw.githubusercontent.com/Aloim/phaneslight/main/Changelog.md -OutFile "$env:TEMP\phaneslight-changelog.md" -ErrorAction Stop; Get-Content "$env:TEMP\phaneslight-changelog.md" -TotalCount 1 } catch { Write-Output "FETCH-FAILED" }
```

Sanity-check both: `phaneslight.md` **MUST** begin with `<!-- PhanesLight v`; `Changelog.md` **MUST** begin with `# Changelog`. If either fetch fails or fails its check, **STOP**: an upgrade without a confirmed target and its changelog is guesswork. Read the target version from the upstream `phaneslight.md` line-1 stamp.
...
**Repository migration (v3.6.1, repeated at v3.7.1, ended at v3.8.0), and the upgrade's duty in it.** v3.6.1 was published to **both** `Aloim/phaneslight` and the legacy `Aloim/phanes`, so an installation still checking the legacy URL saw the release once and could migrate itself. v3.7.1 was published to both again, for the same reason and one more: the Claude Code plugin had moved out to `Aloim/phaneslightplugin`, leaving `Aloim/phaneslight` as the manual line's home. **From v3.8.0 that dual publication stops.** The manual line publishes to `Aloim/phaneslight` alone; the legacy `Aloim/phanes` is frozen at v3.7.1 and receives no further publication, ever. An installation still polling the legacy URL sees v3.7.1 there, is redirected by that release's stub, and upgrades from there. **This upgrade prompt serves the manual installation only**, and a project can be on either line, so check before you touch anything. **If a PhanesLight plugin is installed** (the project resolves `/phaneslight:run`, or `/plugin list` names `phaneslight`, or `.claude/settings.json` carries no PhanesLight hook entries while `.phaneslight/` exists and is populated), **STOP and route the user**, verbatim:
...
1. **This prompt refreshes ITSELF first (v3.8.1), before it replaces anything else.** The target version read in Step 1 is also the upstream `PhanesLightUpgrade.md` stamp, because from v3.3 the two files carry one number (the header of this file). Compare it numerically against this file's own line-1 stamp. If upstream is newer: fetch upstream `PhanesLightUpgrade.md` to a temporary location with the platform variant of the Step 1 command, sanity-check that it begins with `<!-- PhanesLightUpgrade v`, locate the installed `phaneslightupgrade.md` (`<project>/.claude/commands/` first, then `~/.claude/commands/`; the per-project copy wins and the duplication is flagged), copy it to `<same-path>.pre-upgrade`, replace it with the fetched file, tell the user to re-run `/phaneslightupgrade` so the newest upgrade logic executes, and **STOP** there. This was point 2 through v3.8.0, after the `phaneslight.md` replacement, so a stale prompt had already replaced the project's `phaneslight.md` over the installed copy before it discovered it should not have been running, and only the re-run it then triggers replaced it again. The self-check costs nothing wherever it sits and belongs before the first write.
2. Locate the installed `phaneslight.md` command file: `<project>/.claude/commands/phaneslight.md` (per-project), then `~/.claude/commands/phaneslight.md` (user-level). If both exist, the per-project copy wins; flag the duplication to the user. Copy the active one to `<same-path>.pre-upgrade` (moved into the archive once the branch exists), then replace it with the fetched target.
...
* **No PhanesLight detected** (no marker, no `.phaneslight/`, no PhanesLight-pattern `documentation/`) → **STOP**: "No existing PhanesLight installation detected. Run `/phaneslight` for a fresh bootstrap, PhanesLightUpgrade upgrades existing installs only."
* **Already at target version** → **STOP**: "Installation already matches the target spec. Same-version refreshes are `/phaneslight` update runs."
...
**There is no v3.7.0 or v3.7.2 entry on the manual line, for the same reason Section 0 gives: both were plugin-only releases the manual line never published.** `migrationBoundaries` in the target manifest nonetheless declares `3.7.0` as a boundary, and a project at v3.6.1 or below crosses it on this upgrade. **That boundary carries no structural migration step of its own**, because nothing structural moved on the manual line at v3.7.0, it was skipped outright. Its only purpose is to force this full-regeneration path to run at all, so that the v3.7.1 lineup rules and this release's anchor convention land through Phase U3 rather than through an ordinary `/phaneslight` update. **Do NOT go looking for v3.7.0 migration work in this document or in the changelog: there is none to find, and a run that adds a step here to satisfy the boundary has misread its purpose.**
...
| `.phaneslight/scripts/*` | **REGENERATE** | The target spec defines the current script set: whatever `templates/MANIFEST.json` lists with `role: command` for this platform. From v3.4 that set grew on Windows, and from v3.8.0 the same growth lands on POSIX too, so a POSIX project gains on this upgrade exactly the manifest entries it was missing, where earlier upgrades correctly gained none. **(v3.8.1)** A script whose on-disk sha256 differs from the `.phaneslight/manifest.json` record is user-edited: **PRESERVE-and-flag** instead, per Step 3. This row said REGENERATE with no exception through v3.8.0 while Step 3 said the opposite, and a reader working the table alone regenerated over local repairs. |
...
* `OLD ∩ NEW` → **REGENERATE set**, unless the on-disk sha256 differs from the manifest hash → **PRESERVE-and-flag** (hand-customized; migrated only with the user, item by item). **(v3.8.1) This is the only rule and no disposition row overrides it**, `.phaneslight/scripts/*` included. The baseline is `templateSha256` where the manifest records one and the install-time `sha256` where it does not. **A `generatedNotFetched` script (`regen-registry`, `api-diff`) has no `templateSha256` and never will**, because nothing was ever fetched for it; the install-time `sha256` this project recorded when it generated the file is the only baseline there is, and it is enough to tell a local repair from an untouched generation. Preserve it, flag it, and print the existing `CUSTOMIZATION STALE` advisory rather than overwriting it.
...
<!-- LEGACY-NAME-BLOCK BEGIN: the identifiers below are pre-v3.6.0 names, matched literally against what is on disk. Do NOT rename them. -->
   ...
   * **Reinstall the script library** from the target manifest into `.phaneslight/scripts/`, which is what brings in the renamed dispatcher and the new `phaneslight-template` stamps, then smoke-run it: `node .phaneslight/scripts/cli.js module-list`. Do **NOT** delete `.phanes/scripts/` yet.
   ...
   * **Move the command files.** `.claude/commands/phanes.md` becomes `.claude/commands/phaneslight.md`, and `.claude/commands/phanesupgrade.md` becomes `.claude/commands/phaneslightupgrade.md`, each only where present. Delete nothing else under `.claude/commands/`; foreign commands are the user's.
...
12. **Anchor convention sweep (target v3.8.0).** Run `phaneslight anchor-check` over `CLAUDE.local.md` and the active plans under `documentation/plans/`. Rewrite every `UNVERIFIABLE` anchor so it carries an expectation (a bare `path:N` becomes `path:N "<substring expected on that line>"`), content only, never the surrounding prose. **Frozen summaries are untouched**: everything under `documentation/session-summaries/` is a historical record, PRESERVE-classed above, and rewriting one to satisfy a later convention would falsify the record it exists to keep. This is the one-time consumer rewrite, carried here as the step the project's own next upgrade run executes, never as work performed against this prompt's own tree.
...
12. `<projectSlug>-worker` and `<projectSlug>-mechanic` both carry the disclosure obligation (`edits_made` mandatory and exhaustive) naming their spawner. **(target v3.7.1)** The worker carries **escalate-at-MED**; the mechanic carries **escalate-at-LOW**, because it may no longer write code and so cannot absorb even a trivial fix. A mechanic still carrying the worker's MED threshold is a failed migration.
...
15. **Bootstrap command set present and dispatching (target v3.4, POSIX parity from v3.8.0).** Platform-conditional, and the POSIX branch is a real pass rather than a skip. **On Windows:** every command the manifest lists with `role: command` is installed in `.phaneslight/scripts/` and dispatches through the cross-shell entry. Verify by running two of them rather than by listing files, since a file that exists and does not dispatch is the failure this item exists to catch: `node .phaneslight/scripts/cli.js preflight` emits JSON carrying `runType` and `legacyMarkers`, and `node .phaneslight/scripts/cli.js update-preflight` emits a verdict whose `gitDelta` object carries `worktreeDirty`. **On POSIX:** the same two invocations **MUST SUCCEED**, emitting the same keys, with `preflight` reporting `"platform": "posix"`. A refusal by name is now the defect: the manual's POSIX ports land with this release, so a project still refusing these commands by name has a library installed before the port, and the script regeneration item is what repairs it. Either platform's wrong answer reopens the script regeneration item.
16. **Session-start ledger hook present (target v3.4; the on-disk script on both platforms from v3.8.0).** `.claude/settings.json` carries a `SessionStart` hook group invoking `hook-ledger-status`, and its command satisfies the same path discipline as the other two (contains `.phaneslight/scripts/`, no drive letter, no leading slash). **A missing entry on a project installed before v3.4 is a repairable absence, not a user deletion**, and this upgrade adds it; the two are distinguishable because the project's recorded `phanesLightVersion` predates the hook class entirely. **The script itself, not only the hook entry, must be on disk under this platform's extension** (`.ps1` on Windows, `.sh` on POSIX) **on both platforms from v3.8.0**; a POSIX project installed before v3.8.0 that lacks the `.sh` copy is a repairable absence of the same kind, distinguished the same way. Verify the hook body itself is silent on a healthy project: with a closed or absent `.phaneslight/run-progress`, running it prints nothing and exits 0. Silence is this hook's healthy signal, so a hook that speaks on a healthy project is as much a defect as one that stays quiet on a dead run.
...
26. **Repository migration complete (target v3.6.1).** No installed artifact still points at the legacy distribution repository. `grep -r "Aloim/phanes\b"` across `.claude/commands/`, `.claude/agents/` and `.phaneslight/scripts/` returns zero hits; every distribution URL names `Aloim/phaneslight`. An installation left on the legacy URL stops seeing releases and will eventually be checking a repository that holds a different product.
...
32. **Anchor convention sweep verified (target v3.8.0).** Re-run `phaneslight anchor-check` over `CLAUDE.local.md` and the active plans under `documentation/plans/`: no anchor the sweep was supposed to rewrite still reports `UNVERIFIABLE`. `git diff` against the upgrade branch's base commit for every path under `documentation/session-summaries/` shows zero content changes, confirming the sweep left frozen summaries untouched.

33. **Counter and sign-off.** Increment `.claude/.phaneslight`. Present the upgrade branch for user review, the **user** merges; you do not. Close verbatim (do not paraphrase):
