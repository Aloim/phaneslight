<!-- PhanesLightUpgrade v3.8.0, 2026-09-05, production upgrade prompt for existing PhanesLight installations.
...
### Upgrading a manual install: v3.7.1 to v3.8.0, or v3.6.1 to v3.8.0

v3.7.1 is the last manual release before this one, so upgrading from it is the common case. v3.6.1 to v3.8.0 is also a fully supported path now, and not a smaller variant of the common case: as the next section explains, v3.6.1 sits below a boundary this release declares, so it is routed through the same run rather than through an ordinary update.

**There is no manual v3.7.0.** v3.7.0 was the plugin release, published to `Aloim/phaneslight` by mistake; it was built for the Claude community marketplace and has since moved to `Aloim/phaneslightplugin`. Its retirement notice, which told you this path was closing, is withdrawn. A v3.6.1 install goes straight to v3.8.0 and skips nothing that applies to it.

**There is no manual v3.7.2 either.** v3.7.2 was a plugin-only release (POSIX parity for the ten bootstrap commands); the manual line never published it, and its content is folded into this file's `Changelog.md` v3.8.0 entry rather than given a manual number of its own. A v3.7.1 install goes straight to v3.8.0 the same way.
...
**From v3.7.1**, the jump is small and additive: `migrationBoundaries` in the v3.8.0 manifest reads `["3.5.0", "3.7.0"]`, and v3.7.1 sits above the highest entry already, so nothing in your project structure moves on this path. The work is confined to regenerating `<projectSlug>-mechanic` and `<projectSlug>-reviewer` for the two v3.7.1 rules, adding the anchor convention sweep, restating them in the root `CLAUDE.md` Pinned Directives block, and reinstalling the template library at its `v3.8.0` stamps. Accumulated knowledge is untouched.
...
1. Locate the installed `phaneslight.md` command file: `<project>/.claude/commands/phaneslight.md` (per-project), then `~/.claude/commands/phaneslight.md` (user-level). If both exist, the per-project copy wins; flag the duplication to the user. Copy the active one to `<same-path>.pre-upgrade` (moved into the archive once the branch exists), then replace it with the fetched target.
2. Same treatment for the installed `phaneslightupgrade.md` if upstream `PhanesLightUpgrade.md` carries a newer stamp than this file's own line 1: archive, replace, then tell the user to re-run `/phaneslightupgrade` so the newest upgrade logic executes, and **STOP** there.
...
<!-- LEGACY-NAME-BLOCK BEGIN: the identifiers below are pre-v3.6.0 names, matched literally against what is on disk. Do NOT rename them. -->
   ...
   * **Reinstall the script library** from the v3.8.0 manifest into `.phaneslight/scripts/`, which is what brings in the renamed dispatcher and the new `phaneslight-template` stamps, then smoke-run it: `node .phaneslight/scripts/cli.js module-list`. Do **NOT** delete `.phanes/scripts/` yet.
