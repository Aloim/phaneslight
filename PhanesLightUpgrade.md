<!-- PhanesLightUpgrade v3.6.1, 2026-09-03, production upgrade prompt for existing PhanesLight installations.
...
Fetch upstream `phaneslight.md` **AND** `Changelog.md` to temporary locations. **The distribution repository is `Aloim/phaneslight` (v3.6.1)**, moved from the legacy `Aloim/phanes`, which is being handed to a separate and more sophisticated Phanes project. Detect the platform **FIRST** and run only the matching variant:
...
<!-- LEGACY-NAME-BLOCK BEGIN: the identifiers below are pre-v3.6.0 names, matched literally against what is on disk. Do NOT rename them. -->
   ...
   * **Reinstall the script library** from the v3.6.1 manifest into `.phaneslight/scripts/`, which is what brings in the renamed dispatcher and the new `phaneslight-template` stamps, then smoke-run it: `node .phaneslight/scripts/cli.js module-list`. Do **NOT** delete `.phanes/scripts/` yet.
...
11. `<projectSlug>-reviewer` declares `model: fable`, grants no write tools, and states that it produces a plan and hands it back.
...
30. **Counter and sign-off.** Increment `.claude/.phaneslight`. Present the upgrade branch for user review, the **user** merges; you do not. Close verbatim (do not paraphrase):
